import 'package:flutter/material.dart';

TextStyle headerTextStyle = new TextStyle(
    fontSize: 20, fontWeight: FontWeight.bold, color: Colors.grey[850]);
TextStyle positionTextStyle = new TextStyle(
    fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey[700]);
TextStyle subHeaderTextStyle =
    new TextStyle(fontSize: 16, color: Colors.grey[850]);
TextStyle bodyTextStyle = new TextStyle(fontSize: 14, color: Colors.grey);
